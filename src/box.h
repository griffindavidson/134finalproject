 ...
 //     assert(min < max);
	...
	//
	 bool overlap(const Box &box) {
		 Vector3 thisMin = parameters[0];
		 Vector3 thisMax = parameters[1];
		 // this min less than other max and this max greater than other min
		 // if all of these are true then the boxes overlap, otherwise they do not
		 bool overlapX = thisMin.x() <= box.parameters[1].x() && thisMax.x() >= box.parameters[0].x();
		 bool overlapY = thisMin.y() <= box.parameters[1].y() && thisMax.y() >= box.parameters[0].y();
		 bool overlapZ = thisMin.z() <= box.parameters[1].z() && thisMax.z() >= box.parameters[0].z();
